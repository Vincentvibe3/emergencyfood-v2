...
object Marry:GenericCommand(), SlashCommand, MessageCommand {
    ...
    override suspend fun handle(event: MessageReceivedEvent) {
        ...
        if (p1 != null && p2 != null) {
            event.channel.asTextChannel().sendMessage(
                ":heart: ${p1.asMention} is now married to ${p2.asMention} :heart:"
            ).queue()
        }
    }

    override val command: CommandData = Commands.slash(name, "Basically a marriage certificate")
    ...
    override suspend fun handle(event: SlashCommandInteractionEvent) {
        ...
        if (p1 != null && p2 != null) {
            event.reply(
                ":heart: ${p1.asMember?.asMention} is now married to ${p2.asMember?.asMention} :heart:"
            ).queue()
        }
    }
}